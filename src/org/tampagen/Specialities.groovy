...
/**
 * Specialities.groovy
 * Service class responsible for retrieving specialities information for a
 * provider.
 * 
...
class Specialities {
    ...
    Specialities (CloseableHttpClient httpClient, ObjectMapper objectMapper, CStreamConstants constants) {
        this.httpClient = httpClient
        this.objectMapper = objectMapper
        this.constants = constants
    }
    List<String> getSpecialities(String drId, String jwtToken) throws IOException {
        ...
        def url = constants.apiBaseUrl + constants.specialitiesEndpoint
        // Create the request body
        def requestBody = createSpecialitiesFilterRequest(drId)
        ...
        try {
                ...
                if (valueNode != null) {
                    ...
                    if (resultNode != null && resultNode.isArray()) {
                        for (int i = 0; i < resultNode.size(); i++) {
                            def speciality = resultNode.get(i)
                            def speciality_code= speciality.has("Specialty_Code") ? speciality.get("Specialty_Code").asText() : ""
                            specialities.add(speciality_code)
                        }
                    }
                }
            } else {
            ...
            throw new IOException("Error calling specialities filter endpoint for Dr_Id: ${drId}", e)
        }
        return specialities
    }
    private ObjectNode createSpecialitiesFilterRequest(String drId) {
        ...
        def specialitiesFilter = objectMapper.createArrayNode()
        ...
        specialitiesFilter.add(drIdFilter)
        ...
        specialitiesFilter.add(primaryFilter)

        // only Practicing Speciality
        def practicingFilter = objectMapper.createObjectNode()
        practicingFilter.put("Field", "Practicing")
        practicingFilter.put("Operator", "EQUALS")
        practicingFilter.put("Value", "true")
        specialitiesFilter.add(practicingFilter)

        //mainFilters.add(specialitiesFilter)

        mainFilter.set("Filters", specialitiesFilter)
